...
O resultado da construção da infraestrutura com o `serverless framework` é exibido na image 14, onde são mostrados os endpoints para acesso pelo **Postman**. Caso fosse realizado qualquer alteração nos arquivos, seria necessário executar o comando `serverless deploy` novamente. Se fosse apenas alterada uma função lambda, o comando executado era o `serverless deploy -f insertItem`. Nas imagens 15, 16 e 17 é possível visualizar, respectivamente, o banco de dados de nome `ItemTableNew` (ou, `ItemTable`) no serviço DynamoDB, as cinco funções no serviço Lambda e a API de nome `dev-projectDioServerless`.
...
A próxima etapa foi acessar o site do **Postman** para realizar as requisições. Nele, foi criado um workspace de nome `projectDioServerless` e elaborado as requisições com base nas funções Lambda criadas, cujo objetivo era executar um **CRUD (Create, Read, Update e Delete)** na tabela do banco construída. A primeira requisição foi a `insertItem` que usou o método `POST` na URL `https://n1xd6v8tp7.execute-api.us-east-1.amazonaws.com/dev/item` para inserir o elemento na tabela do banco de dados. A segunda requisição foi a `listItems` utilizando o método `GET` na URL `https://n1xd6v8tp7.execute-api.us-east-1.amazonaws.com/dev/items` para retornar todos os elementos do banco de dados com seus atributos. A terceira requisição de nome `getItem`, também utilizou o método `GET`, porém no final da URL era informado o `id` do elemento do banco de dados que deveria ser retornado. A última requisição foi a `updateItem` que utilizou o método `PUT` na mesma URL da requisição anterior, onde o objetivo era alterar o valor de `itemStatus` para `true` do elemento de `id` indicado na URL. Apenas os métodos `POST` e `PUT` teve um corpo na requisição. No arquivo [requisicoes.txt](./automation/resources/requisicoes.txt) estão os exemplos utilizados nas requisições do **Postman**. As próximas quatro imagens a seguir (19, 20, 21 e 22) ilustram a execução das requisições no **Postman**.
...
Caso seja necessário acessar o ambiente remotamente, uma alternativa utilizada ao invés do **PuTTY** foi com o comando `ssh -i "G:\Meu Drive\4_PROJ\boot\dio\aws\boot_011\03-aws_foundation\automation\secrets\remoteAccessEc2.pem" ubuntu@ec2-18-234-136-69.compute-1.amazonaws.com`, informando o local do arquivo par de chave `.pem` e o endereço completo da maquina virtual. Observando que o número de IP é separado por traços (`-`) e não por pontos (`.`). Este comando utiliza o software **OpenSSH** para realizar o acesso remoto e pode ser feito diretamente do CLI utilizado, que no caso, foi do **PowerShell**.