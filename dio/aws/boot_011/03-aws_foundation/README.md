...
Neste desafio, para que não fossem feitas instalações na maquina física **Windows**, foi montado um ambiente para execução do projeto em uma maquina **Linux Ubuntu** instância no serviço **Amazon Elastic Compute Cloud (EC2)** da cloud da **AWS**. O ambiente foi criado através do diretório [automation](./automation/) que contém três scripts de **PowerShell** ([criacao](./automation/criacao.ps1), [exclusao](./automation/exclusao.ps1), [variaveis](./automation/variaveis.ps1)) destinados a construção e exclusão desta instância e de um par de chaves para acesso remoto à ela. Este diretório ainda contém duas sub-pastas de nome [resources](./automation/resources/), com um único arquivo de script em **Bash** para execução assim que a maquina fosse instanciada, e `secrets`, que armazenava qualquer conteúdo sigiloso, neste caso, ele armazenou dois arquivos par de chaves criado nos formatos `.pem`, para chave pública e `.ppk` para chave privada, utilizados para acesso remoto. O script de criacao verificou se já existiam o par de chaves e a maquina virtual que desejava-se construir, caso não, ele realizava toda criação com as configurações determinadas e indicando que arquivo de script Bash deveria ser executado no Linux assim que a instância tivesse pronta. Já o script de exclusão verificou se existiam o par de chaves e a maquina virtual, caso sim, executava a exclusão desses dois serviços. O script de variaveis serviu apenas para determinar nomes para as variaveis utilizadas nestes dois arquivos, portanto foi necessário importá-la neles.
...
A minha execução foi diferente, com o script `criacao` em execução, após construção da maquina virtual Linux Ubuntu, foi aguardado um tempo determinado até que a instância obtivesse um IP público disponível e que fosse gerado, manualmente via software **Puttygen**, a chave privada `.ppk` a partir da chave pública `.pem` fornecida pela cloud da AWS. Então com o software **PSCP**, que é um cliente SCP, ou seja, cópia segura de arquivo de linha de comando, foi realizado a transferência da pasta [projectDioServerless](./projectDioServerless/), com todos os arquivos, da maquina física Windows para o diretório `home/ubuntu` da instância no serviço EC2 da cloud. Em seguida, foi executado o acesso remoto a instância via **PuTTY**, com uso da chave privada `.ppk` para verificar se todos os pacotes foram instalados corretamente conforme o script `ec2Script.sh`. Também nesse acesso, foi configurado a **AWS CLI** para acessar cloud da AWS com o usuário administrador da conta `PedroheegerAdmin`. Utilizando o comando `aws configure` foi informado as seguintes credenciais: **AWS Access Key ID** e **AWS Secret Key** deste usuário, a região que seria utilizada, que no caso foi `us-east-1`, e a última informação foi mantida vazia. Para conferir o usuário logado utilizou-se o comando `aws sts get-caller-identity` que exibiu as credenciais cadastradas.
