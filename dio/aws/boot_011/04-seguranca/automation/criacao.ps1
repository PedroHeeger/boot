...
if ((aws dynamodb describe-table --table-name $tableName --region $region 2>&1) -match "TableNotFoundException"){
...
"-----//-----//-----//-----//-----//-----//-----"
Write-Output "INTEGRATION METHOD AND DEPLOYMENT"
# Write-Output "API GATEWAY"
if ((aws apigateway get-rest-apis --query "items[?name=='$apiGatewayName'].name").Count -gt 1) {
    ...
    if ((aws apigateway get-resources --rest-api-id $apiId --query "items[?pathPart=='$resourceApiName'].id").Count -gt 1) {
        ...
        if ((aws apigateway get-method --rest-api-id $apiId --resource-id $resourceId --http-method POST).Count -gt 1) {
            ...
            "-----//-----//-----//-----//-----//-----//-----"
            Write-Output "INTEGRATION METHOD (RESPONSE)"
            if ((aws lambda get-policy --function-name $lambdaFunctionName 2>&1) -match "NotFoundException"){
                Write-Output "A configuração de resposta para o método POST do recurso $resourceApiName não foi encontrada!"
                ...
                if ((aws apigateway get-method --rest-api-id $apiId --resource-id $resourceId --http-method POST --query "methodResponses").Count -gt 1) {
