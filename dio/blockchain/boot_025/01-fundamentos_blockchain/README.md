...
### Development:
O desenvolvimento deste módulo do bootcamp foi dividido em dois cursos, um desafio de projeto e um desafio de código. Abaixo é explicado o que foi desenvolvido em cada uma dessas atividades.

<a name="item1.1"><h4>1.1 Introdução à Blockchain</h4></a>[Back to summary](#item1) | <a href="https://github.com/PedroHeeger/main/blob/main/cert_ti/04-curso/blockchain/(24-08-14)_Introducao...Blockchain_PH_DIO.pdf">Certificate</a>
...
<a name="item1.2"><h4>1.2 Desafio de Projeto: Criando e Utilizando a Sua Carteira de Criptomoedas</h4></a>[Back to summary](#item1) | <a href="a">Certificate</a>

Neste desafio de projeto, o objetivo foi através de arquivos de código em **JavaScript** gerar uma carteira de **Bitcoin**, importar essa carteira para um software gerenciador de carteiras (Wallet) e realizar de transações de envio e recebimento de **Bitcoin**. Foi utilizado um faucet para **Bitcoin**, que é um serviço que distribui pequenas quantidades de criptomoeda gratuitamente para usuários, geralmente para fins de teste ou para incentivar o uso e a adoção da criptomoeda. Ele pode oferecer moedas reais na mainnet ou moedas sem valor real na testnet. Neste caso, ele foi utilizado para obter moedas na testnet da blockchain do **Bitcoin**, que é uma rede auxiliar para testes. A wallet escolhida foi a **Electrum Bitcoin Wallet** que teve que ser instalada na máquina física **Windows**. 

Uma ideia minha foi tentar utilizar a carteira da **Blockchain.com**, pois esse site é bem completo, já que além da carteira, ele funciona como exchange e explorador de blocos, sendo utilizado pelo navegador sem a necessidade de fazer instalação. Contudo, ele só funcionava na mainnet e era necessário criar uma conta e validar informações pessoais para obter acesso total ao software, não fornecendo o anonimato que o software **Electrum Bitcoin Wallet** fornecia. Como explorador de blocos, ele também só funcionava na mainnet. Como alternativa foi utilizado o explorador de blocos **Blockstream Explorer**, que funcionava na testnet.

Com relação a parte de codificação, para não ter que instalar o **Node.js** na máquina física, foi utilizado uma instância do **Amazon Elastic Compute Cloud (Amazon EC2)**, que é um serviço da cloud **AWS** para provisionamento de máquinas virtuais. A instância foi definida como do tipo `t2.micro`, com a imagem de máquina (AMI) `ami-0c7217cdde317cfec` (Canonical, Ubuntu, 22.04 LTS, amd64 jammy image build on 2023-12-07) baseada em **Linux Ubuntu**. Um arquivo de user data foi usado para instalar previamente **Node.js** e **NPM**, além de ferramentas básicas do **Linux**. O par de chaves `keyPairUniversal` foi utilizado. Este era um par de chaves já criado na minha conta da **AWS** para projetos de estudo, no qual o arquivo de chave privada já estava salvo na máquina física. O provisionamento da instância foi gerenciado por um arquivo em **PowerShell** com comandos **AWS CLI**, dividido em dois scripts, um para criação e outro para exclusão, e estruturado com condições que aguardavam a entrada do usuário para executar o código. A **AWS CLI** estava previamente instalada e configurada na máquina física com o usuário administrador da **AWS** (`PedroHeegerAdmin`). O acesso remoto à instância foi realizado usando o software **OpenSSH**, já instalado na máquina física. Para garantir a comunicação, regras de entrada foram configuradas no grupo de segurança vinculado à instância, incluindo a liberação da porta `22` do protocolo `TCP` para acesso remoto. Essa instância utilizou o security group padrão da região em que foi provisionada para agilizar o processo. Dessa forma, com a execução do arquivo [ec2Instance.ps1](./02-dp/ec2Instance.ps1) a instância do **Amazon EC2** foi provisionada na **AWS**.

Para realização do acesso remoto foi executado o comando `ssh -i "G:/Meu Drive/4_PROJ/scripts/scripts_model/.default/secrets/awsKeyPair\keyPairUniversal.pem" ubuntu@54.89.207.96`, passando o IP ou DNS público da instância, o usuário que iria fazer o acesso remoto, que no caso era o usuário `ubuntu` já que era uma máquina **Linux Ubuntu**, e também foi passado o caminho para o arquivo de chave privada para a autenticação desse usuário. Para confirmar que o **Node.js** e o **NPM** tinham sido instalados corretamente foi executado no terminal da instância os comandos `node --version` e `npm --version`. A imagem 02 mostra os dois softwares instalados na instância. Algumas das bibliotecas instaladas do **Node.js** dependiam do **Node.js** ser uma versão mais recente para seu funcionamento. Por isso, foi instalada a versão 22 do **Node.js** que já instalava o **NPM** conjuntamente.
...
Dando sequência, no diretório do usuário foi criada uma pasta específica para o projeto com o comando `mkdir btcwallet`. Em seguida, essa pasta foi acessada com o comando `cd btcwallet`. Nesta pasta que seriam construídos os arquivos em **JavaScript** com o **Node.js**. Assim, o comando `npm init -y` foi executado para iniciar um projeto **Node.js**. Este criou o arquivo de configuração `package.json`, onde as dependências (bibliotecas) para o projeto eram indicadas. Neste projeto foram utilizadas as seguintes bibliotecas: **bip39**, **bip32** e **bitcoinjs-lib**. Para instalá-las foi executado o comando `npm install bip39 bip32@2.0.6 bitcoinjs-lib --save`, sendo o **bit32** instalado na versão `2.0.6`. 

Após isso, foi criada a pasta `src` com o comando `mkdir src` e nela foi construído o arquivo [createWallet.js](./02-dp/createWallet.js) utilizando o editor de texto **Nano**. Este arquivo realizou toda criação da carteira, sendo uma wallet do tipo determinística hierárquica (HD Wallet). Então primeiro foi definido a rede que seria utilizada, no caso a testnet. Em seguida, a forma de derivação da carteira foi definida como `m/49'/1'/0'/0`, que era para rede testnet. Caso fosse para mainnet, tinha que ser `m/49'/0'/0'/0`. Logo depois, foi criado a frase mnemônica, que é uma sequência de palavras usadas para gerar e recuperar chaves criptográficas em carteiras de criptomoedas, facilitando a memorização e a recuperação dos fundos. Ela é traduzida em uma sequência binária que pode ser usada para derivar todas as chaves e endereços de uma Hierarchical Deterministic Wallet (HD Wallet). Com a frase mnemônica foi criada a seed e com o seed e a rede foi gerado a chave mestre, também chamada de raiz da carteira HD.

A partir da raiz, uma conta foi criada indicando a forma de derivação da carteira definida anteriormente. Com essa conta, múltiplos pares de chaves (privados e públicos) poderiam ser criados. Ao executar `account.derive(0).derive(0)` indicava a primeira chave pública e privada da conta, ou seja, o primeiro par de chaves foi selecionado. A partir desse par de chaves, a chave pública foi extraída e junto com a rede especificada foi construído um endereço vinculado a chave pública desse par de chaves. Esse endereço construído foi do tipo `P2WPKH (SegWit - Bech32)`, mas também existia o `P2SH (Pay to Script Hash)` e o `P2PKH (Pay to Public Key Hash)`, sendo esse último o tipo indicado pelo expert do curso. Contudo, este indicado pelo professor não funcionava ao tentar utilizar o faucet **Bitcoin Testnet Faucet** para enviar bitcoins para a carteira, informando que a transação era inválida. Por isso, foi gerado o endereço do tipo `P2WPKH (SegWit - Bech32)`. Por fim, os elementos criados necessários para as próximas etapas foram exibidos, conforme mostrado na imagem 03.
...
Após a criação da carteira por código **JavaScript**, uma segunda carteira foi construída, mas agora no software **Electrum Bitcoin Wallet** que tinha sido baixado anteriormente. O **Electrum** possuía dois executáveis, um para acessar o software na mainnet e outro para a testnet, sendo esse último de nome *Electrum Testnet*. Ao iniciar qualquer um dos executáveis, ele solicitava que um nome fosse fornecido para carteira ou escolhesse um arquivo de uma carteira já existente no computador. O nome foi determinado como `dio-wallet` e em seguida, era questionado que tipo de carteira seria construída, sendo mantida a opção carteira padrão. Após isso, ele perguntava se gostaria de criar um novo seed ou informar um seed ou chave mestra existente. Como era a primeira carteira, foi mantida a opção criar um novo seed e então a frase mnemônica foi fornecida com 12 palavras. Essa informação tinha que ser salva, pois permitia recuperar a carteira em caso de falha. Em seguida, ele pedia exatamente para confirmar o seed, ou seja, a frase mnemônica copiada. Após confirmação, era solicitado a criação de uma senha para criptografar o arquivo da carteira. Com toda essa configuração feita já era possível acessar a carteira `dio-wallet` que no momento não tinha nada, nenhum saldo e nenhuma transação, conforme mostrado na imagem 04. Essa carteira foi criada na mainnet só para entender como era o processo de construção de uma carteira no software **Electrum**. Ela foi excluída posteriormente, pois a ideia principal do desafio era importar a carteira criada pelo arquivo de código.
...
Como esse desafio seria na testnet, foi executado o *Electrum Testnet*, que poderia ser chamado no terminal informando o path para sua pasta acrescido de `--testnet`. Agora, ao invés de criar uma nova carteira, a primeira carteira, criada pelo arquivo de código **JavaScript**, foi importada, nomeando ela como `dio-wallet-boot_025`. Portanto, foi escolhido a opção importar endreço de bitcoin e então foi solicitada a chave privada da carteira. Essa chave privada foi gerada pelo código e exibida no output junto com as outras informações. Ao passar a chave privada no **Electrum** foi necessário adicionar antes da chave o prefixo `p2wpkh:`. Esse prefixo foi o mesmo utilizado no código e indicava o tipo de endereço vinculado a uma chave pública específica da carteira. Isso foi necessário, porque o **Electrum**, por padrão, utiliza um tipo de endereço específico e para que ele utilizasse o tipo de endereço gerado no arquivo de código, esse prefixo tinha que ser indicado. Após indicação da chave privada com o prefixo, foi preciso apenas criar uma senha para criptografar o arquivo da carteira, sendo ela `dio-boot_025-test1`. Todas as outras informações, como seed, frase mnemônica, pares de chaves, endereços já tinham sido criados pelo arquivo de código, não sendo necessário criá-los novamente, pois tudo era importado. A imagem 05 exibe a carteira importada no **Electrum Bitcoin Wallet** na testnet, ainda sem nenhuma transação ou saldo.
...
Para realizar as transações foi utilizado o faucet **Bitcoin Testnet Faucet**, onde era possível passar um endereço e enviar uma quantidade de **Bitcoin**. Lembrando que como era na testnet, esses bitcoins não tinham valor. O endereço informado foi o mesmo gerado no arquivo de código em **Node.js** que utilizou o tipo `P2WPKH`. Esse endereço estava vinculado a chave pública de um dos pares de chaves de uma das contas da carteira criada no arquivo de código e importada para o **Electrum**. A imagem 06 evidencia a transação realizada. Já a imagem 07 mostra o saldo, bem como a transação na carteira já confirmada pela blockchain.
...
Ainda foi possível utilizar o explorador de blocos **Blockstream Explorer** para visualizar as transações na testnet sendo confirmadas nos blocos da rede blockchain. O explorador de blocos **Blockchain.com** só funcionava para mainnet. A imagem 08 exibe umas das duas transações realizadas, enquanto a imagem 09 mostra o endereço que recebeu as transações, que no caso foi o endereço gerado a partir de uma das chaves públicas da carteira importada no **Electrum**. Observe que no endereço foi exibido as duas transações realizadas, cada uma com sua identificação, evidenciando de onde e para onde a moeda foi enviada.
...
Por fim, o código em **Node.js** foi executado novamente, agora para obter um segundo endereço, consequentemente de outra carteira. Mas nesse caso, o objetivo foi só enviar o saldo existetente na carteira importada no **Electrum** para um endereço qualquer, que no caso foi esse gerado. Todo o saldo foi enviado e confirmado com a senha cadastrada. A imagem 10 exibe as informações da transferência realizada. Já na imagem 11 é evidenciado a transação no explorador de blocos **Blockstream Explorer**. Note que a transação ainda não tinha sido confirmada na blockchain.
...
<a name="item1.3"><h4>1.3 Operações da Blockchain</h4></a>[Back to summary](#item1) | <a href="https://github.com/PedroHeeger/main/blob/main/cert_ti/04-curso/blockchain/(24-08-15)_Operacoes...Blockchain_PH_DIO.pdf">Certificate</a>
...
<a name="item1.4"><h4>1.4 Desafio de código: Desafios intermediários Javascript I - Formação Blockchain</h4></a>[Back to summary](#item1)
